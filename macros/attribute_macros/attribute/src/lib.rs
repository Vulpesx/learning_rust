...
/// Parses a list of variable names by commas.
///
///     a, b, c
///
...
impl Args {
    /// Determins whether the given `Expr` is a path to one of the
    /// variables we intend to print. Expressions are used as the left-hand side
    /// of the assignement operator.
    ...
    /// Determins whether the given `Pat` is an identifier equil to one of the
    ...
    ///
    ///     // After
    ///     let VAR = { let VAR = INIT; println!("VAR = {:?}", VAR); };
...
/// The `Fold` trait is a way to traberse an owner syntax tree ans replace some
/// of its nodes.
///
/// Syn provides to other syntax tree traversal traits: `Visit` which walks a
...
/// which we want non-default behavour. In this case the traversal needs to 
...
pub fn trace_vars(args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    // Parse the input into a syntax tree of a function
